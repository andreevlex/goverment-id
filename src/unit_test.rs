...
fn test_invalid_inn_9zeros() {
    assert!(Inn::new("000000000").is_valid() == Err("ИНН должен быть длиной 10 или 12 цифр".to_string()));
}
...
fn test_invalid_inn_11zeros() {
    assert!(Inn::new("00000000000").is_valid() == Err("ИНН должен быть длиной 10 или 12 цифр".to_string()));
}
...
fn test_invalid_inn_too_short() {
    assert!(Inn::new("772053").is_valid() == Err("ИНН должен быть длиной 10 или 12 цифр".to_string()));
}
...
fn test_invalid_inn_with_litters() {
    assert!(Inn::new("782f004526").is_valid() == Err("ИНН должен состоять только из цифр".to_string()));
}
...
fn test_invalid_kpp_7_numbers() {
    assert!(Kpp::new("01234567").is_valid() == Err("КПП может состоять только из 9 знаков (цифр или заглавных букв латинского алфавита от A до Z)".to_string()));
}
...
fn test_invalid_kpp_10_numbers() {
    assert!(Kpp::new("0123456789").is_valid() == Err("КПП может состоять только из 9 знаков (цифр или заглавных букв латинского алфавита от A до Z)".to_string()));
}
